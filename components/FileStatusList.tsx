  ...
  return (
    <div className="mb-8 bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
      ...
      <div className="divide-y divide-gray-100">
        {files.map((file) => (
          <div key={file.id} className="px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3 overflow-hidden">
              <div className={`p-2 rounded-lg flex-shrink-0 ${file.status === 'ERROR' ? 'bg-red-50' : 'bg-blue-50'}`}>
                {file.status === 'ERROR' ? (
            ...
            <div className="flex items-center gap-2 self-start sm:self-center flex-shrink-0">
              ...
              {file.status === 'ERROR' && (
                <div className="flex items-center text-red-700 bg-red-50 px-3 py-1 rounded-full text-sm font-bold border border-red-100">
                  Try Again
                </div>
              )}
            </div>
